...
#
# pc = np.load('cps_x.npy')
# y = np.load('cps_y.npy')
# # PC_Vis.draw_pc_labels(pc[:, :3], y, io.get_cfg_params(), True)
#
# # scan_file = 'cps_sample.laz'
# #
# # las_data = laspy.read(scan_file)
# # pc = np.vstack([las_data.x, las_data.y, las_data.z, las_data.intensity]).transpose()
#
# pcd = o3d.geometry.PointCloud()
# pcd.points = o3d.utility.Vector3dVector(pc[:, :3])
# min_bound = pcd.get_min_bound()
# max_bound = pcd.get_max_bound()
# downpcd = pcd.voxel_down_sample_and_trace(voxel_size=0.2, min_bound=min_bound, max_bound=max_bound)
#
# X = np.asarray(downpcd[0].points)
# inds0 = downpcd[1]
# print(inds0[:2, :])
# inds = np.ravel(inds0)
# # print(inds[:16])
# val_inds = np.where(inds != -1)[0]
# # print(inds[val_inds][:16])
# inds[val_inds] = y[val_inds]
# vox_y = np.reshape(inds, (-1, 8))
# print(vox_y)
# vox_y = vox_y.astype(float)
# # val_inds = np.where(inds != -1)[0]
# # print(val_inds)
# # print(np.where(inds0 != -1))
# # val_y = y[val_inds]
# # inds[val_inds] = val_y
# # vox_y = np.reshape(inds, (-1, 8)).astype(float)
# # vox_y[vox_y == -1] = np.nan
# # print(vox_y)
#
# from scipy import stats
# # m = stats.mode(vox_y, axis=1)
# # mapped_y = m[0]
# m = stats.tmin(vox_y, -1, 1, False)
# mapped_y = m
# print(mapped_y)
# # Y = np.squeeze(mapped_y).astype(np.int32)
#
# PC_Vis.draw_pc_labels(X[:, :3], mapped_y.astype(int), io.get_cfg_params(), True)